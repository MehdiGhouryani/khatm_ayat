...
from bot.handlers.settings_handlers import (
    ...
    lock_on, lock_off, jam_off, jam_on, set_completion_message, reset_daily, reset_off, 
    reset_number_on, reset_number_off, delete_after, delete_off,
...
from bot.database.db import get_db_connection, init_db, process_queue_request
...
async def queue_worker():
    """Process write requests from the queue with error handling."""
    while True:
        try:
            request = await write_queue.get()
            logger.debug("Processing queue request: %s", request)
            await process_queue_request(request)
            write_queue.task_done()
            logger.info("Queue request processed, queue size: %d", write_queue.qsize())
        except Exception as e:
            logger.error("Error in queue worker: %s, request=%s", e, request, exc_info=True)
            write_queue.task_done()
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.debug("User %s cancelled conversation in chat %s", 
...
def register_jobs(app: Application):
    ...
    try:
        ...
        logger.error("Error registering jobs: %s", e, exc_info=True)
        raise
...
def main():
    ...
    try:
        ...
        loop.create_task(queue_worker())  # Start queue worker
